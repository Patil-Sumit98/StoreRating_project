...
const AddStorePage = () => {
  ...
  const [ownerId, setOwnerId] = useState(''); // 👈 New state for the selected owner
  
  const [users, setUsers] = useState([]); // 👈 New state for the list of users
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // 👇 New useEffect to fetch all users for the dropdown
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      // 👇 Include ownerId in the data sent to the backend
    ...
    <div style={styles.container}>
      <div style={styles.formContainer}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* 👇 New Dropdown for selecting an owner */}
          <div style={styles.inputGroup}>
