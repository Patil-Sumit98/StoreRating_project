...
const StoreDetail = () => {
  const { id } = useParams();
  const { token, user } = useAuth(); // 👈 Get the full user object
  ...
  // 👇 Determine if the logged-in user is the owner of this store
  ...
  const handleRatingSubmit = async () => {
    ...
    try {
      ...
      // Refresh the store data to show the new average rating
      await fetchStore();
      setNewRating(0);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.ratingSection}>
        ...
        {/* 👇 Updated conditional logic to check for ownership */}
        {isOwner ? (
...
const styles = {
  ...
  // 👇 New style for the owner message
  ownerMessage: {
    textAlign: 'center',
    fontSize: '1.1rem',
    color: '#7f8c8d',
